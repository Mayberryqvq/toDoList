...
import com.mayberry.todolist.databinding.AddTagDialogBinding
...
class AddTagDialogFragment: DialogFragment() {
    ...
    override fun onCreateView(
        ...
        binding = AddTagDialogBinding.inflate(layoutInflater)
        return binding.root
    }
    ...
    private fun saveTag() {
        if (binding.tagNameEditText.text.isEmpty()) {
            ...
            mViewModel.insertTag(TagData(0, binding.tagNameEditText.text.toString(), bgColor))
            dismiss()
        }
    }

    private fun colorViewSelected(view: View) {
        binding.underline.animate().translationX(view.x - binding.color1.x).setDuration(500).start()
